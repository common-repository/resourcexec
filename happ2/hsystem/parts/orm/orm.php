...
abstract class _HC_ORM extends _HC_MVC
{
		...
		// if( $this->related === NULL ){
			...
			if( $relman ){
				...
				foreach( $this->related as $k => $v ){
					if( in_array($v->type(), array('has_many', 'belongs_to_many')) ){
						$join = $v->join();
						if( ! $join ){
							$join = new _HC_ORM_WordPress_Custom_Post_Storable();
							$join->post_type = $this->app_name() . '-' . $k;
							$this->related[$k]->set_join( $join );
						}
					}
				}
			}
	...
	public function fetch_array( $fields = '*' )
	{
		...
		$where = $this->where;
		$return = $this->storable->fetch( $fields, $where, $this->limit, $order_by );

		$single_field = ( is_array($fields) OR ($fields == '*') ) ? FALSE : TRUE;
		if( $single_field ){
			$new_return = array();
			foreach( $return as $id => $row ){
				$new_return[] = $row[$fields];
			}
			$return = $new_return;
		}
...
// PROPERTIES
	...
	public function to_array( $with_related = 1 ) // 0 - no, 1 - full, 2 - flat
	{
